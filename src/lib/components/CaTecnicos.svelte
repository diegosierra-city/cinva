<script lang="ts">
	...
	console.log(
		urlAPI +
			...
			'&token=' +
			$userNow.token +
			'&folder=ca_tecnicos&campo=tipo&campoV=tecnico&orden=nombre'
	);
	onMount(async () => {
		await fetch(
			urlAPI +
	...
	const upload = (e: any) => {
		...
		if (image_id > 1000000) {
			...
			dataArray.append('folder', 'ca_tecnicos');
									...
									{:else if tecnico.imagen != ''}
										<div class="flex items-center">
											<img
												...
												src="{urlFiles}imagenes/ca_tecnicos/M{tecnico.imagen}"
												alt={tecnico.nombre}
											/>
