<script lang="ts">
	...
	export let listProveedores: Proveedor[]
	...
	const upload = async (position: number) => {
		...
		await fetch(urlAPI + '?ref=upload&folder=cinva_products&prefix=', {
			method: 'POST',
			body: dataArray
		})
			.then((response) => response.json())
	...
	const uploadOption = async (e: any) => {
		...
		await fetch(urlAPI + '?ref=upload&folder=cinva_product_versions&prefix=', {
			method: 'POST',
			body: dataArray
		})
			.then((response) => response.json())
	...
	const loadOptions = async (p: number) => {
		console.log(urlAPI +
				'?ref=list&f=cinva_product_versions&id=' +
				p +
				'&user_id=' +
				$userNow.id +
				'&time=' +
		...
		await fetch(
			urlAPI +
				'?ref=load-list&folder=cinva_product_versions&campo=product_id&campoV=' +
				p +
				'&user_id=' +
				$userNow.id +
				'&time_life=' +
			...
			.then((result) => {
				console.log('options load:',result);
					...
					//if(result.lenght>0) listOptions = result;
				listOptions = result;
			})
			.catch((error) => console.log(error.message));
	};
	...
	const saveOption = () => {
		...
		//fetch(urlAPI + '?ref=save-options', {
			...
			.then((result) => {
				//listOptions = result;
					console.log('ListOptions:',result)
					...
					<div>
						<div class="md:flex items-center lg:ml-24">
							<div class="md:w-64 mx-2">
								...
								{:else if prod.image1 != ''}
									<img src="{urlFiles}/images/cinva_products/M{prod.image1}" alt={prod.product} />
									<br />
									<button
										class="btn-red flex"
										on:click={() => {
							...
							<div class="md:w-64 mx-2">
								{#if prod.image2 == 'load'}
									<Circle3 size="60" unit="px" duration="2s" />
								{:else if prod.image2 != ''}
									<img src="{urlFiles}/images/cinva_products/M{prod.image2}" alt={prod.product} />
									<br />
									<button
										class="btn-red flex"
										on:click={() => {
						...
						<div class="md:flex items-center lg:ml-24">
							<div class="md:w-64 mx-2">
								{#if prod.image3 == 'load'}
									<Circle3 size="60" unit="px" duration="2s" />
								{:else if prod.image3 != ''}
									<img src="{urlFiles}/images/cinva_products/M{prod.image3}" alt={prod.product} />
									<br />
									<button
										class="btn-red flex"
										on:click={() => {
							...
							<div class="md:w-64 mx-2">
								{#if prod.image4 == 'load'}
									<Circle3 size="60" unit="px" duration="2s" />
								{:else if prod.image4 != ''}
									<img src="{urlFiles}/images/cinva_products/M{prod.image4}" alt={prod.product} />
									<br />
									<button
										class="btn-red flex"
										on:click={() => {
...
<div class="md:w-64 mx-2">
	...
	{#if listProveedores.length>0}
		 <!-- content here -->
			<select class="inputA" bind:value={prod.proveedor_id}>
				<option value="">Seleccione</option>
				{#each listProveedores as prov}
				 <!-- content here -->
					<option value="{prov.id}">{prov.nombre}-{prov.ciudad}</option>
			{/each}
			</select>
		...
		/>
			...
			<div class="xl:px-8">
				<div class="mt-16 lg:flex justify-between border-b border-silver pb-4">
					<div class="w-80">
						<div class="flex items-center">
							<h1 class="text-xl font-medium pr-2 leading-5 text-dimgray">Costo / Existencias</h1>
						</div>
					...
					<div>
						<div class="md:flex items-center lg:ml-24">
							<div class="md:w-64 mx-2">
								Costo<br />
								<input type="number" class="inputA" placeholder="Costo" bind:value={prod.price} />
							</div>
<div class="md:w-64 mx-2">
								Unidad <small>(unidades,cajas,mts...)</small><br />
								<input type="text" class="inputA" placeholder="Unidad" bind:value={prod.unidad} />
							</div>
							<div class="md:w-64 mx-2">
								Existencias<br />
								<input type="number" class="inputA w-24" placeholder="Existencias" bind:value={prod.stock} />
							</div>
						</div>
					</div>
				</div>
			...
			<div class="xl:px-8">
				<div class="mt-16 border-b border-silver pb-4">
					<div class="w-80">
						<div class="flex items-center">
						...
						<div class="md:flex items-center">
							<table>
								<thead>
									...
									<th scope="col" class="px-2 py-1"> Costo </th>
											...
											<td class="">
												...
												{:else if option.image != ''}
													<div class="relative">
														<img
															class="w-16 h-auto "
															src="{urlFiles}/images/cinva_product_versions/M{option.image}"
															alt={option.name}
														/>
